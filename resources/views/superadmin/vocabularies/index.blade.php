...
<body class="bg-neutral-100 font-sans">
    <div x-data="{
        ...
        modalTitle: '',
        formAction: '',
        vocab: { lesson_id: '', category: '', items: [{ term: '', details: [{chunk: '', translation: ''}], media_url: null }] },

        openModal(existingVocabulary = null) {
            if (existingVocabulary) {
                this.isEditMode = true;
                this.modalTitle = 'Edit Vocabulary';
                this.formAction = `/superadmin/vocabularies/${existingVocabulary.id}`;
                this.vocab = JSON.parse(JSON.stringify(existingVocabulary));
            } else {
                this.isEditMode = false;
                this.modalTitle = 'Buat Vocabulary Baru';
                this.formAction = '{{ route('superadmin.vocabularies.store') }}';
                this.vocab = { lesson_id: '', category: '', items: [{ term: '', details: [{chunk: '', translation: ''}] }] };
            }
            this.isModalOpen = true;
        },

        addItem() {
            this.vocab.items.push({term: '', details: [{chunk: '', translation: ''}], media_url: null});
        },
        removeItem(index) {
            this.vocab.items.splice(index, 1);
        },

        // --- PERBAIKAN: Fungsi untuk menambah dan menghapus details ---
        addDetail(itemIndex) {
            if (!this.vocab.items[itemIndex].details) {
                this.vocab.items[itemIndex].details = [];
            }
            this.vocab.items[itemIndex].details.push({ chunk: '', translation: '' });
        },
        removeDetail(itemIndex, detailIndex) {
            if (this.vocab.items[itemIndex] && this.vocab.items[itemIndex].details) {
                this.vocab.items[itemIndex].details.splice(detailIndex, 1);
            }
        }
    }">
                        ...
                        <template x-for="(item, index) in vocab.items" :key="index">
                            <div class="item-group border-t pt-4">
                                ...
                                <div class="mt-4 p-4 border bg-gray-50 rounded-md">
                                    <label class="block text-sm font-medium text-neutral-700 mb-2">Details (Penggalan Teks & Terjemahan)</label>
                                    <template x-if="!item.details || item.details.length === 0">
                                        <div x-init="item.details = [{chunk: '', translation: ''}]"></div>
                                    </template>
                                    <template x-for="(detail, detailIndex) in item.details" :key="detailIndex">
                                        <div class="grid grid-cols-2 gap-2 mb-2 items-center">
                                            <input type="text" :name="`items[${index}][details][${detailIndex}][chunk]`" x-model="detail.chunk" placeholder="Penggalan Teks (EN)" class="px-3 py-2 border border-neutral-300 rounded-md">
                                            <div class="flex items-center gap-2">
                                                <input type="text" :name="`items[${index}][details][${detailIndex}][translation]`" x-model="detail.translation" placeholder="Terjemahan (ID)" class="flex-1 px-3 py-2 border border-neutral-300 rounded-md">
                                                <button type="button" @click="removeDetail(index, detailIndex)" class="text-red-500 font-bold">&times;</button>
                                            </div>
                                        </div>
                                    </template>
                                    <button type="button" @click="addDetail(index)" class="text-xs text-indigo-600">+ Tambah Penggalan</button>
                                </div>

                                <div class="text-right mt-2"><button type="button" @click="vocab.items.splice(index, 1)" class="text-sm font-medium text-red-600 hover:text-red-800">Hapus Item</button></div>
                            </div>
                        </template>
                    </div>
                    <button type="button" @click="vocab.items.push({term: '', details: [{chunk: '', translation: ''}], media_url: null})" class="mt-4 bg-green-500 text-white py-2 px-4 rounded-lg text-sm font-semibold hover:bg-green-600"><i class="fas fa-plus mr-2"></i>Tambah Item</button>
